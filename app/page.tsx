    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/20 bg-white/85 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
        <div className="flex items-center gap-3">
          <div className="grid h-12 w-12 place-items-center rounded-2xl bg-brand-700 text-xl font-black text-white shadow-lg">P</div>
          <div>
            <p className="text-xl font-black tracking-[0.18em] text-brand-900">PAPAIPAY</p>
            <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-slate-500">Plan Advisor · Pay Advisor</p>
          </div>
        </div>
        <nav className="hidden items-center gap-8 text-sm font-bold text-slate-700 lg:flex">
          {navItems.map((item) => <a key={item} href="#" className="hover:text-brand-700">{item}</a>)}
        </nav>
        <div className="hidden items-center gap-3 lg:flex">
          <button className="rounded-full border border-slate-200 px-4 py-2 text-sm font-bold text-slate-700">BM | EN</button>
          <a href="#mohon" className="rounded-full bg-brand-700 px-5 py-3 text-sm font-black text-white shadow-lg">Mohon Sekarang</a>
        </div>
    ...
    <main className="min-h-screen bg-[#f7fbf8] text-slate-950">
      ...
      <section className="relative min-h-screen overflow-hidden bg-gradient-to-br from-brand-900 via-brand-700 to-emerald-500 pt-24 text-white">
        ...
        <div className="relative mx-auto grid min-h-[calc(100vh-96px)] max-w-7xl items-center gap-10 px-5 py-16 lg:grid-cols-[1.08fr_0.92fr] lg:px-8">
          <div className="max-w-3xl">
            <div className="mb-5 inline-flex rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs font-black uppercase tracking-[0.25em] backdrop-blur">Papaipay Financial Advisory</div>
            ...
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              <a href="#mohon" className="rounded-full bg-white px-7 py-4 text-center text-sm font-black text-brand-900 shadow-2xl">Mohon Sekarang</a>
              <a href="#henry" className="rounded-full border border-white/30 px-7 py-4 text-center text-sm font-black text-white backdrop-blur">Ketahui Pendekatan Kami</a>
            </div>
          </div>
      ...
      <section className="px-5 py-20 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-5 md:grid-cols-3">
          {trustCards.map((item) => (
            <div key={item} className="rounded-[2rem] border border-emerald-100 bg-white p-8 shadow-xl shadow-emerald-950/5">
      ...
      <section id="henry" className="bg-white px-5 py-24 lg:px-8">
        ...
        <div className="mx-auto mt-14 grid max-w-6xl gap-4 md:grid-cols-5">
          {["Fahami Keperluan", "Semak Pilihan", "Dapatkan Bimbingan", "Susun Keputusan", "Pantau Susulan"].map((step, index) => (
            <div key={step} className="rounded-[1.6rem] bg-slate-50 p-6 text-center">
      ...
      <section id="mohon" className="px-5 py-24 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-10 rounded-[2.5rem] bg-white p-6 shadow-2xl shadow-emerald-950/10 md:p-10 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <form className="grid gap-4">
            ...
            <button type="button" className="rounded-full bg-brand-700 px-7 py-4 text-sm font-black text-white shadow-lg">Hantar Permohonan</button>
          </form>
        </div>
      </section>
    </main>
