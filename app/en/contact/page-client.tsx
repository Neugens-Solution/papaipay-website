...
const contactCards = [
  {
    title: "Contact Us",
    value: "03-6279 0080",
    description: "Speak with our team during office hours for enquiries and appointment assistance.",
  },
  {
    title: "Email Us",
  ...
  },
  {
    ...
    description: "Visit our office for scheduled consultation sessions and appointments.",
  },
  {
    title: "Operating Hours",
    value: "Monday – Friday\n10:00 AM – 6:00 PM\n\nSaturday – Sunday\nClosed",
    description: "Appointments are encouraged so our team can prepare for your consultation session.",
  },
];
    ...
    <main className="min-h-screen bg-[#f7fbf8] text-slate-950">
      ...
      <section className="relative flex min-h-[190px] items-center justify-center overflow-hidden pt-16 text-center text-white md:min-h-[320px] md:pt-16">
        ...
        <div className="relative mx-auto w-full max-w-7xl px-5 py-8 md:py-16 lg:px-8">
          <h1 className="text-3xl font-bold leading-[1.05] tracking-[-0.05em] md:text-4xl lg:text-5xl">Contact Us</h1>
        </div>
      </section>

      <section className="px-5 py-16 lg:px-8 lg:py-20">
        <div className="mx-auto max-w-3xl text-center">
          <p className="text-xs font-extrabold uppercase tracking-[0.28em] text-brand-700">Connect With Papaipay</p>
          <h2 className="mt-4 text-3xl font-bold leading-[1.04] tracking-[-0.045em] text-slate-950 md:text-5xl">We Are Ready To Help</h2>
          <p className="mt-5 text-base leading-8 text-slate-600 md:text-lg">Contact Papaipay for enquiries, consultation sessions and appointment arrangements with our team.</p>
        </div>
      </section>
        ...
        <div className="mx-auto grid max-w-7xl overflow-hidden rounded-[2.25rem] border border-emerald-100 bg-[#f7fbf8] shadow-2xl shadow-emerald-950/10 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="relative overflow-hidden bg-[linear-gradient(135deg,#082314_0%,#0b301d_55%,#031a0e_100%)] p-8 text-white md:p-12">
            ...
            <div className="relative">
              <span className="inline-flex rounded-full border border-[#d6b95f]/30 bg-white/10 px-4 py-2 text-[11px] font-extrabold uppercase tracking-[0.24em] text-[#d6b95f]">ENQUIRY FORM</span>
              <h2 className="mt-6 text-3xl font-bold leading-[1.04] tracking-[-0.045em] md:text-5xl">Share Your Details With Our Team</h2>
              <p className="mt-5 max-w-xl text-base leading-8 text-white/74">Complete this form and our team will review your enquiry before contacting you with the next steps.</p>
              <ul className="mt-8 grid gap-3 text-sm font-semibold text-white/82">
                {['Professional advisory support', 'Clearer next steps', 'Document guidance if needed'].map((item) => <li key={item} className="flex items-center gap-3"><span className="h-2 w-2 rounded-full bg-[#d6b95f]" />{item}</li>)}
              </ul>
              <div className="mt-10 rounded-2xl border border-white/10 bg-white/10 p-5 text-sm font-semibold leading-7 text-white/78">For urgent enquiries, please call us during office hours.</div>
            </div>
          </div>
          <form onSubmit={(event) => event.preventDefault()} className="p-6 md:p-10">
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <label className="grid gap-2 text-sm font-extrabold text-slate-800"><span>Service of Interest</span><select className="h-14 rounded-2xl border border-emerald-100 bg-white px-5 text-sm font-semibold text-slate-800 outline-none transition focus:border-brand-700 focus:ring-4 focus:ring-brand-700/10">{enquiryTypes.map((type) => <option key={type}>{type}</option>)}</select></label>
            </div>
            <label className="mt-5 grid gap-2 text-sm font-extrabold text-slate-800"><span>Message</span><textarea rows={6} placeholder="Briefly tell us about your enquiry." className="resize-none rounded-2xl border border-emerald-100 bg-white px-5 py-4 text-sm font-semibold text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-brand-700 focus:ring-4 focus:ring-brand-700/10" /></label>
            <p role="status" className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-bold leading-7 text-amber-900">This online form is being prepared. For immediate enquiries, please contact the Papaipay team by phone or email.</p><a href="tel:0362790080" className="mt-7 inline-flex w-full justify-center rounded-full bg-brand-700 px-8 py-4 text-xs font-extrabold uppercase tracking-[0.14em] text-white shadow-xl shadow-emerald-950/15 transition hover:-translate-y-0.5 hover:bg-brand-900 md:w-auto">Contact Us</a>
          </form>
        </div>
      </section>
      ...
      <section className="bg-[#f7fbf8] px-5 py-20 lg:px-8 lg:py-24">
        <div className="mx-auto max-w-7xl">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h2 className="mt-4 text-3xl font-bold leading-[1.04] tracking-[-0.045em] text-slate-950 md:text-5xl">Visit Our Office</h2>
            <p className="mt-5 text-base leading-8 text-slate-600 md:text-lg">Our office is located in Bandar Sri Damansara and is available for scheduled consultation sessions and appointments.</p>
          </div>
      ...
      <section className="bg-white px-5 py-20 lg:px-8 lg:py-24">
        <div className="mx-auto max-w-5xl text-center">
          ...
          <h2 className="mt-4 text-3xl font-bold leading-[1.04] tracking-[-0.045em] text-slate-950 md:text-5xl">Stay connected with Papaipay.</h2>
      ...
      <section className="bg-white px-5 pb-20 lg:px-8 lg:pb-24">
        <div className="relative mx-auto max-w-7xl overflow-hidden rounded-[2rem] bg-[linear-gradient(135deg,#082314_0%,#0b301d_52%,#031a0e_100%)] p-8 text-center text-white shadow-2xl shadow-emerald-950/20 md:p-14">
          ...
          <div className="relative">
            <h2 className="mx-auto max-w-3xl text-3xl font-bold leading-tight tracking-[-0.045em] md:text-5xl">Speak With Us</h2>
            <p className="mx-auto mt-6 max-w-2xl text-base leading-8 text-white/78">Connect with our team to learn more about the services we provide.</p>
            <a href="tel:0362790080" className="mt-9 inline-flex rounded-full bg-white px-8 py-4 text-xs font-extrabold uppercase tracking-[0.12em] text-brand-900 shadow-xl transition hover:-translate-y-0.5 hover:bg-white/90">Contact Us Now</a>
          </div>
        </div>
      </section>
