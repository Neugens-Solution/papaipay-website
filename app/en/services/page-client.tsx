...
export default function ServicesPage() {
  return <main className="min-h-screen bg-[#f7fbf8] text-slate-950"><SiteHeader active="services" locale="en" />
    ...
    <section className="relative overflow-hidden bg-[#02140c] px-5 py-20 text-white lg:px-8 lg:py-24"><div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_16%,rgba(31,143,77,0.36),transparent_34%),radial-gradient(circle_at_84%_24%,rgba(214,185,95,0.18),transparent_30%)]" /><div className="relative mx-auto max-w-7xl"><SectionHeading light title="From Challenges to Solutions" intro="Papaipay helps customers review their financial position through a more structured, clear and responsible approach." /><div className="mt-12 grid gap-5 lg:grid-cols-2"><div className="rounded-[2rem] border border-white/10 bg-white/10 p-8 backdrop-blur-md"><h3 className="text-2xl font-extrabold tracking-[-0.04em]">Common Challenges</h3><ul className="mt-7 grid gap-4">{challenges.map((item) => <li key={item} className="flex gap-3 text-white/78"><span className="mt-1 h-2.5 w-2.5 shrink-0 rounded-full bg-[#d6b95f]/70" />{item}</li>)}</ul></div><div className="rounded-[2rem] border border-white/10 bg-white/10 p-8 backdrop-blur-md"><h3 className="text-2xl font-extrabold tracking-[-0.04em]">Papaipay Approach</h3><ul className="mt-7 grid gap-4">{approaches.map((item) => <li key={item} className="flex gap-3 text-white/82"><CheckIcon muted />{item}</li>)}</ul></div></div></div></section>
    ...
    <section className="bg-white px-5 py-20 lg:px-8 lg:py-24"><div className="mx-auto max-w-7xl overflow-hidden rounded-[2rem] bg-[#082314] p-8 text-center text-white shadow-2xl shadow-emerald-950/20 md:p-14"><p className="text-xs font-extrabold uppercase tracking-[0.3em] text-[#d6b95f]">Contact Papaipay</p><h2 className="mx-auto mt-5 max-w-3xl text-3xl font-extrabold leading-tight tracking-[-0.045em] md:text-5xl">Need Guidance on Your Financial Options?</h2><p className="mx-auto mt-6 max-w-2xl text-base leading-8 text-white/72">Our team is ready to help you understand suitable options based on your current financial situation and future goals.</p><a href="/en/contact" className="mt-9 inline-flex rounded-full bg-white px-8 py-4 text-xs font-extrabold uppercase tracking-[0.12em] text-brand-900 shadow-xl transition hover:-translate-y-0.5 hover:bg-white/90">Contact Us</a></div></section>
