...
export default function TeamPage() {
  return <main className="min-h-screen bg-[#f7fbf8] text-slate-950"><SiteHeader active="team" locale="en" />
    ...
    <section className="bg-[#f7fbf8] px-5 py-20 lg:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><SectionHeading title="Executive Management" description="Executive Management sets Papaipay’s strategic direction and ensures its operations and finances grow sustainably." /><div className="mx-auto mt-12 grid max-w-4xl gap-6 md:grid-cols-2">{executiveManagementGroup.map((member) => <TeamCard key={member.name} member={member} featured />)}</div></div></section>
    <section className="bg-white px-5 py-20 lg:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><SectionHeading title="Team Leadership" description="Team Leadership guides daily performance, team development and the customer experience." /><div className="mx-auto mt-12 grid max-w-5xl gap-6 md:grid-cols-3">{teamLeadershipGroup.map((member) => <TeamCard key={member.name} member={member} featured />)}</div></div></section>
    <section className="bg-[#f7fbf8] px-5 py-20 lg:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><SectionHeading title="Financial Planning Team" description="The Financial Planning Team helps customers understand their options and plan financial steps suited to their needs." /><div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-5">{financialPlanningGroup.map((member) => <TeamCard key={member.name} member={member} />)}</div></div></section>
    <section className="bg-white px-5 py-20 lg:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><SectionHeading title="Operations & Support" description="Operations & Support keeps administration, people, communications and daily processes running smoothly." /><div className="mx-auto mt-12 grid max-w-6xl gap-6 sm:grid-cols-2 lg:grid-cols-4">{operationsSupportGroup.map((member) => <TeamCard key={member.name} member={member} />)}</div></div></section>
