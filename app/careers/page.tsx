...
const cultureItems = [
  ...
  ["target", "Customer-focused mindset"],
];
        ...
        <div className="mx-auto grid max-w-7xl overflow-hidden rounded-[2.25rem] border border-emerald-100 bg-white shadow-2xl shadow-emerald-950/10 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="relative overflow-hidden bg-[linear-gradient(135deg,#082314_0%,#0b301d_55%,#031a0e_100%)] p-8 text-white md:p-12">
            ...
            <div className="relative">
              ...
              <h2 className="mt-6 text-3xl font-extrabold leading-[1.04] tracking-[-0.045em] md:text-5xl">Submit Your Resume</h2>
              ...
              <ul className="mt-8 grid gap-3 text-sm font-semibold text-white/82">
                {['Open for motivated individuals', 'Professional growth opportunities', 'Shortlisted candidates will be contacted'].map((item) => <li key={item} className="flex items-center gap-3"><span className="h-2 w-2 rounded-full bg-[#d6b95f]" />{item}</li>)}
              </ul>
              <div className="mt-10 rounded-2xl border border-white/10 bg-white/10 p-5 text-sm font-semibold leading-7 text-white/78">Our team will review your submission and contact shortlisted candidates.</div>
            </div>
          </div>
          <form className="bg-[#f7fbf8] p-6 md:p-10">
            ...
            <label className="mt-5 grid gap-2 text-sm font-extrabold text-slate-800"><span>Message</span><textarea rows={5} required placeholder="Tell us briefly about your experience and career interest." className="resize-none rounded-2xl border border-emerald-100 bg-white px-5 py-4 text-sm font-semibold text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-brand-700 focus:ring-4 focus:ring-brand-700/10" /></label>
