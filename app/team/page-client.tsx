...
export default function TeamPage() {
  return <main className="min-h-screen bg-[#f7fbf8] text-slate-950"><SiteHeader active="team" />
    ...
    <section className="bg-[#f7fbf8] px-5 py-20 lg:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><SectionHeading title="Kepimpinan Pasukan" description="Kepimpinan Pasukan membimbing prestasi harian, pembangunan ahli dan pengalaman pelanggan." /><div className="mx-auto mt-12 grid max-w-5xl gap-6 md:grid-cols-3">{teamLeadershipGroup.map((member) => <TeamCard key={member.name} member={member} featured />)}</div></div></section>
    <section className="bg-white px-5 py-20 lg:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><SectionHeading title="Pasukan Perancangan Kewangan" description="Pasukan Perancangan Kewangan membantu pelanggan memahami pilihan dan merancang langkah kewangan yang sesuai dengan keperluan mereka." /><div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-5">{financialPlanningGroup.map((member) => <TeamCard key={member.name} member={member} />)}</div></div></section>
    <section className="bg-[#f7fbf8] px-5 py-20 lg:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><SectionHeading title="Operasi & Sokongan" description="Pasukan Operasi & Sokongan memastikan pentadbiran, sumber manusia, komunikasi dan proses harian berjalan lancar." /><div className="mx-auto mt-12 grid max-w-6xl gap-6 sm:grid-cols-2 lg:grid-cols-4">{operationsSupportGroup.map((member) => <TeamCard key={member.name} member={member} />)}</div></div></section>
