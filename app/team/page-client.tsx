...
export default function TeamPage() {
  return <main className="min-h-screen bg-[#f7fbf8] text-slate-950"><SiteHeader active="team" />
    ...
    <section className="bg-white px-5 py-20 lg:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><SectionHeading title="Pasukan Penasihat" description="Pasukan penasihat yang membantu pelanggan memahami pilihan dan peluang yang sesuai dengan keperluan mereka." /><div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-5">{advisoryTeam.map((member) => <TeamCard key={member.name} member={member} />)}</div></div></section>
    <section className="bg-[#f7fbf8] px-5 py-20 lg:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><SectionHeading title="Pasukan Operasi" description="Pasukan operasi yang memastikan proses harian berjalan lancar dan pengalaman pelanggan kekal konsisten." /><div className="mx-auto mt-12 grid max-w-5xl gap-6 md:grid-cols-3">{operationsTeam.map((member) => <TeamCard key={member.name} member={member} featured />)}</div></div></section>
