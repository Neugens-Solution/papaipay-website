...
export default function TeamPage() {
  return <main className="min-h-screen bg-[#f7fbf8] text-slate-950"><SiteHeader active="team" />
    <section className="relative flex min-h-[190px] items-center justify-center overflow-hidden pt-16 text-center text-white md:min-h-[320px] md:pt-16"><div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: "url('/hero-team.png')" }} /><div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(3,26,14,0.9)_0%,rgba(3,26,14,0.72)_48%,rgba(3,26,14,0.88)_100%)]" /><div className="absolute inset-0 bg-gradient-to-t from-[#031a0e]/82 via-black/10 to-black/30" /><div className="relative mx-auto w-full max-w-7xl px-5 py-8 md:py-16 lg:px-8"><h1 className="text-3xl font-bold leading-[1.05] tracking-[-0.05em] md:text-4xl lg:text-5xl">Pasukan Kami</h1></div></section>
    ...
    <section className="bg-[#f7fbf8] px-5 py-20 lg:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><SectionHeading title="Pengurusan Eksekutif" description="Pengurusan Eksekutif menetapkan hala tuju strategik serta memastikan operasi dan kewangan Papaipay berkembang secara mampan." /><div className="mx-auto mt-12 grid max-w-4xl gap-6 md:grid-cols-2">{executiveManagementGroup.map((member) => <TeamCard key={member.name} member={member} featured />)}</div></div></section>
    <section className="bg-white px-5 py-20 lg:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><SectionHeading title="Kepimpinan Pasukan" description="Kepimpinan Pasukan membimbing prestasi harian, pembangunan ahli dan pengalaman pelanggan." /><div className="mx-auto mt-12 grid max-w-5xl gap-6 md:grid-cols-3">{teamLeadershipGroup.map((member) => <TeamCard key={member.name} member={member} featured />)}</div></div></section>
    <section className="bg-[#f7fbf8] px-5 py-20 lg:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><SectionHeading title="Pasukan Perancangan Kewangan" description="Pasukan Perancangan Kewangan membantu pelanggan memahami pilihan dan merancang langkah kewangan yang sesuai dengan keperluan mereka." /><div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-5">{financialPlanningGroup.map((member) => <TeamCard key={member.name} member={member} />)}</div></div></section>
    <section className="bg-white px-5 py-20 lg:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><SectionHeading title="Operasi & Sokongan" description="Pasukan Operasi & Sokongan memastikan pentadbiran, sumber manusia, komunikasi dan proses harian berjalan lancar." /><div className="mx-auto mt-12 grid max-w-6xl gap-6 sm:grid-cols-2 lg:grid-cols-4">{operationsSupportGroup.map((member) => <TeamCard key={member.name} member={member} />)}</div></div></section>
    <section className="relative overflow-hidden bg-[#02140c] px-5 py-20 text-white lg:px-8 lg:py-24"><div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_16%,rgba(31,143,77,0.36),transparent_34%),radial-gradient(circle_at_84%_24%,rgba(214,185,95,0.18),transparent_30%),linear-gradient(135deg,rgba(255,255,255,0.05)_0_1px,transparent_1px_24px)]" /><div className="relative mx-auto max-w-7xl"><SectionHeading light title="Rangkaian Penasihat Kami" description="Pengalaman, kepakaran dan rangkaian penasihat kami membantu lebih ramai pelanggan membuat keputusan kewangan dengan lebih yakin." /><div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{networkStats.map(([value, label]) => <div key={label} className="rounded-[1.75rem] border border-white/10 bg-white/10 p-7 text-center shadow-2xl shadow-black/10 backdrop-blur-md"><p className="text-3xl font-extrabold leading-tight tracking-[-0.05em] text-white md:text-4xl">{value}</p><p className="mt-4 text-xs font-extrabold uppercase tracking-[0.2em] text-[#d6b95f]">{label}</p></div>)}</div></div></section>
    <section className="bg-white px-5 py-20 lg:px-8 lg:py-24"><div className="relative mx-auto max-w-7xl overflow-hidden rounded-[2rem] bg-[linear-gradient(135deg,#082314_0%,#0b301d_52%,#031a0e_100%)] p-8 text-center text-white shadow-2xl shadow-emerald-950/20 md:p-14"><div className="absolute inset-0 bg-cover bg-center opacity-[0.1]" style={{ backgroundImage: "url('/hero-team.png')" }} /><div className="absolute inset-0 bg-black/25" /><div className="relative"><h2 className="mx-auto max-w-3xl text-3xl font-bold leading-tight tracking-[-0.045em] md:text-5xl">Sedia Berhubung Dengan Pasukan Kami?</h2><p className="mx-auto mt-6 max-w-2xl text-base leading-8 text-white/78">Hubungi Papaipay untuk memahami pilihan kewangan yang sesuai berdasarkan situasi dan keperluan anda.</p><a href="/contact" className="mt-9 inline-flex rounded-full bg-white px-8 py-4 text-xs font-bold uppercase tracking-[0.12em] text-brand-900 shadow-xl transition hover:-translate-y-0.5 hover:bg-white/90">Hubungi Kami</a></div></div></section>
    <SiteFooter /></main>;
}
