...
function ServiceTabs() {
  ...
  return <section className="bg-white px-5 py-20 lg:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="Perkhidmatan" title="Penyelesaian Kewangan Yang Tersusun" intro="Setiap perkhidmatan direka untuk membantu pelanggan memahami kedudukan kewangan mereka sebelum membuat keputusan yang lebih jelas dan bertanggungjawab." />
    <div className="mt-12 hidden md:block"><div className="flex gap-3 rounded-[2rem] border border-emerald-100 bg-[#f7fbf8] p-3">{services.map((item, index) => <button key={item.title} onClick={() => setActive(index)} className={`rounded-[1.35rem] px-5 py-4 text-sm font-extrabold transition md:flex-1 md:text-center ${active === index ? "bg-brand-700 text-white shadow-lg shadow-emerald-950/15" : "bg-white text-slate-700 hover:text-brand-700"}`}>{item.title}</button>)}</div><article className="mt-8 grid gap-8 rounded-[2rem] border border-emerald-100 bg-white p-5 shadow-2xl shadow-emerald-950/5 md:p-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center"><div><p className="text-xs font-extrabold uppercase tracking-[0.28em] text-brand-700">{service.label ?? "Khidmat Nasihat Papaipay"}</p><h3 className="mt-4 text-3xl font-extrabold leading-tight tracking-[-0.045em] text-slate-950 md:text-4xl">{service.title}</h3><p className="mt-5 max-w-2xl text-base leading-8 text-slate-600">{service.description}</p><ul className="mt-8 grid gap-4">{service.bullets.map((bullet) => <li key={bullet} className="flex items-start gap-3 text-sm font-semibold text-slate-700"><CheckIcon /> <span>{bullet}</span></li>)}</ul></div><div className="overflow-hidden rounded-[1.5rem] bg-emerald-950/5"><img src={service.image} alt={service.title} className="aspect-[5/3] h-full w-full object-cover" /></div></article></div>
