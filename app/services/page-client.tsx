...
const services = [
  ...
  {
    title: "Pengurusan Pembiayaan Peribadi",
  ...
  },
  {
    ...
    label: "Pendekatan Khas Papaipay",
    image: "/Henry-Asset-Approach.png",
    description: "Pendekatan khas Papaipay yang memberi fokus kepada bagaimana komitmen kewangan boleh diurus secara lebih strategik bagi membuka ruang kepada pembinaan aset dan perancangan jangka panjang.",
    bullets: ["Fokus kepada pembangunan aset", "Perancangan kewangan yang lebih mampan", "Membantu pelanggan merancang peluang pemilikan aset masa hadapan"],
  },
];
...
const processSteps = [
  ...
  ["04", "Panduan & Tindakan Susulan", "Kami membantu pelanggan melalui proses seterusnya dan memberi panduan sehingga tindakan susulan selesai."],
];
...
function ServiceTabs() {
  ...
  return <section className="bg-white px-5 py-20 lg:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="Perkhidmatan" title="Penyelesaian Kewangan Yang Tersusun" intro="Setiap perkhidmatan direka untuk membantu pelanggan memahami kedudukan kewangan mereka sebelum membuat keputusan yang lebih jelas dan bertanggungjawab." />
    <div className="mt-12 hidden md:block"><div className="flex gap-3 rounded-[2rem] border border-emerald-100 bg-[#f7fbf8] p-3">{services.map((item, index) => <button key={item.title} onClick={() => setActive(index)} className={`rounded-[1.35rem] px-5 py-4 text-sm font-bold transition md:flex-1 md:text-center ${active === index ? "bg-brand-700 text-white shadow-lg shadow-emerald-950/15" : "bg-white text-slate-700 hover:text-brand-700"}`}>{item.title}</button>)}</div><article className="mt-8 grid gap-8 rounded-[2rem] border border-emerald-100 bg-white p-5 shadow-2xl shadow-emerald-950/5 md:p-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center"><div><p className="text-xs font-bold uppercase tracking-[0.28em] text-brand-700">{service.label ?? "Khidmat Nasihat Papaipay"}</p><h3 className="mt-4 text-3xl font-bold leading-tight tracking-[-0.045em] text-slate-950 md:text-4xl">{service.title}</h3><p className="mt-5 max-w-2xl text-base leading-8 text-slate-600">{service.description}</p><ul className="mt-8 grid gap-4">{service.bullets.map((bullet) => <li key={bullet} className="flex items-start gap-3 text-sm font-semibold text-slate-700"><CheckIcon /> <span>{bullet}</span></li>)}</ul></div><div className="relative aspect-[5/3] overflow-hidden rounded-[1.5rem] bg-emerald-950/5"><Image src={service.image} alt={service.title} fill sizes="(min-width: 1024px) 36vw, 100vw" className="object-cover" /></div></article></div>
    <div className="mt-10 grid gap-4 md:hidden">{services.map((item, index) => { const open = active === index; return <article key={item.title} className={`overflow-hidden rounded-[1.5rem] border transition ${open ? "border-brand-700/35 bg-white shadow-xl shadow-emerald-950/10" : "border-emerald-100 bg-[#f7fbf8] shadow-lg shadow-emerald-950/5"}`}><button type="button" onClick={() => setActive(index)} aria-expanded={open} className="flex w-full items-center justify-between gap-4 px-5 py-5 text-left"><span><span className="block text-[11px] font-extrabold uppercase tracking-[0.22em] text-brand-700">Perkhidmatan 0{index + 1}</span><span className="mt-1 block text-base font-extrabold leading-tight tracking-[-0.025em] text-slate-950">{item.title}</span></span><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-full border transition ${open ? "border-brand-700 bg-brand-700 text-white" : "border-emerald-100 bg-white text-brand-700"}`}><ChevronIcon open={open} /></span></button>{open && <div className="border-t border-emerald-100 px-5 pb-6 pt-5"><div className="relative aspect-[5/3] overflow-hidden rounded-[1.1rem] bg-emerald-950/5"><Image src={item.image} alt={item.title} fill sizes="100vw" className="object-cover" /></div>{item.label && <p className="mt-5 text-[11px] font-extrabold uppercase tracking-[0.24em] text-brand-700">{item.label}</p>}<p className="mt-4 text-sm leading-7 text-slate-600">{item.description}</p><ul className="mt-5 grid gap-3">{item.bullets.map((bullet) => <li key={bullet} className="flex items-start gap-3 text-sm font-semibold leading-6 text-slate-700"><CheckIcon /> <span>{bullet}</span></li>)}</ul></div>}</article>; })}</div>
  </div></section>;
}

export default function ServicesPage() {
  return <main className="min-h-screen bg-[#f7fbf8] text-slate-950"><SiteHeader active="services" />
    <section className="relative flex min-h-[190px] items-center justify-center overflow-hidden pt-16 text-center text-white md:min-h-[320px] md:pt-16"><div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: "url('/hero-service.png')" }} /><div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(3,26,14,0.9)_0%,rgba(3,26,14,0.72)_48%,rgba(3,26,14,0.88)_100%)]" /><div className="absolute inset-0 bg-gradient-to-t from-[#031a0e]/82 via-black/10 to-black/30" /><div className="relative mx-auto w-full max-w-7xl px-5 py-8 md:py-16 lg:px-8"><h1 className="text-3xl font-bold leading-[1.05] tracking-[-0.05em] md:text-4xl lg:text-5xl">Perkhidmatan Kami</h1></div></section>
    <ServiceTabs />
